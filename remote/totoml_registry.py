...
def check_registry(model_name, correlation_id): 
    '''
    ...
    if not 'name' in model: 
        ...
        if response.status_code != 201: 
            logger.compute(correlation_id, 'Something went wrong when creating a new model on Toto ML Model Registry. Response: {content}'.format(content=response.content), 'error')
        else: 
